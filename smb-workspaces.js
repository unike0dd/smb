(() => {
  ...
  const settingsMarkup = () => {
    ...
    const wizard = `<div class="employee-wizard" id="employeeWizard" hidden><div class="wizard-dialog"><button class="wizard-close" id="closeEmployeeWizard" type="button" aria-label="Close employee setup">×</button><header><h3>Add employee</h3><div class="wizard-steps"><span>1<small>Details</small></span><i></i><span>2<small>Reporting &amp; team</small></span><i></i><span class="active">3<small>Access</small></span><i></i><span>4<small>Review &amp; invite</small></span></div></header><div class="wizard-grid"><main><span>Step 3 of 4</span><h3>Set access &amp; working hours</h3><p>Choose a role preset to apply recommended permissions, then adjust where needed.</p><div class="role-presets"><button class="active" type="button"><strong>Manager</strong><small>Full operational access</small></button><button type="button"><strong>Supervisor</strong><small>Oversees teams and operations</small></button><button type="button"><strong>Staff</strong><small>Limited to assigned tasks</small></button></div>${access}</main><aside><h3>Employee summary</h3>${employee}<div class="risk-summary"><strong>Access risk check</strong><span>Permission level <b>Low risk</b></span><span>Approve permissions <b class="moderate">Moderate</b></span><span>Sensitive areas <b>Low risk</b></span></div></aside></div></div></div>`;
  ...
  const render = (screen, requestedTab) => {
    ...
    if (screen === "settings") {
      ...
      $("openEmployeeWizard")?.addEventListener("click", () => { wizard.hidden = false; document.body.classList.add("employee-wizard-open"); });
      ...
      host.querySelectorAll("[data-open-access]").forEach(button => button.addEventListener("click", () => { wizard.hidden = false; document.body.classList.add("employee-wizard-open"); }));
      ...
      host.querySelectorAll("[data-person-index]").forEach(row => row.addEventListener("click", () => host.querySelectorAll("[data-person-index]").forEach(item => item.classList.toggle("selected", item === row))));
