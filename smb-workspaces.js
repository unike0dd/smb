(() => {
  ...
  const SETTINGS_SECTIONS = [
    ...
    ["Access", [["access-security", "Create Access and Security", "Create and authorize employee access records."]]],
  ...
  const PEOPLE = {
    "Upper Management": [["Jeff Lou Sedfniv", "VP"], ["Lu Revnic", "Director"]],
    "Middle Management": [["Shifty Lowel", "Manager"], ["Maggie Leveeh", "Manager"]],
    "Field Management": [["Charlie Viznu", "Supervisor"], ["Sue Ischmoo", "Supervisor"]],
    "Staff": [["Luther McVeil", "Staff"], ["Art McDwell", "Staff"]]
  };
  ...
  const settingsPane = key => {
    ...
    return `<h3>Payroll</h3><p>Authorize payroll work only for employees with an approved business need.</p>${guide("Open an employee, verify the task, permission, device and MFA, then authorize with the owner password.")}<div class="access-list">${Object.values(PEOPLE).flat().map(person => accessRecord(person, false)).join("")}</div>`;
  };
