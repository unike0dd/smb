(() => {
  ...
  const settingsMarkup = active => `<div class="settings-layout"><nav class="settings-tree" aria-label="Settings sections">${SETTINGS_SECTIONS.map(group => `<section><h3>${group[0]}</h3>${group[1].map(item => `<button class="${item[0] === active ? "active" : ""}" type="button" data-settings-section="${item[0]}"><span><strong>${item[1]}</strong><small>${item[2]}</small></span><b>›</b></button>`).join("")}</section>`).join("")}</nav><article class="workspace-card settings-pane">${settingsPane(active)}</article></div>`;
  ...
  const render = (screen, requestedTab) => {
    ...
    if (screen === "settings") {
      ...
      host.innerHTML = `<div class="workspace-shell"><header class="workspace-toolbar"><div><h2>Settings</h2><p>Profiles, staff access, delegated workspaces, activity and payroll authorization.</p></div></header>${settingsMarkup(setting)}</div>`;
