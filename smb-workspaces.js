(() => {
  ...
  const SETTINGS_SECTIONS = [
    ["Profile", [["my-profile", "My Profile", "Update your personal sign-in and security details."], ["business-profile", "Business Profile", "Maintain company identity and activate employees."]]],
    ["Access", [["user-access", "User Access", "Manage 25 users across two accountable teams."], ["access-security", "Create Access and Security", "Create and authorize employee access records."]]],
    ["Delegation", [["delegated-staff", "Staff Setting", "Browse employees by level and assigned RBAC."], ["delegated-workspace", "Workspace", "Create workspaces and assign employee access."], ["activity", "Activity", "Review employee login activity."], ["payroll", "Payroll", "Control payroll-specific access and security."]]]
  ];
  const PEOPLE = {
  ...
  const guide = text => `<small class="option-guide">What to do: ${text}</small>`;
  const compactOption = (title, help, action = "Manage") => `<article class="setting-option"><div><b>${title}</b>${guide(help)}</div><button type="button" data-workspace-action="${title} opened">${action}</button></article>`;
  const staffControls = person => `<div class="employee-controls"><label>Assign task<input aria-label="Assign task to ${person}" placeholder="Enter task"></label><label>Salary access<select aria-label="Salary access for ${person}"><option>No access</option><option>View</option><option>Manage</option></select></label><label>Device access<select aria-label="Device access for ${person}"><option>Approved devices</option><option>Any managed device</option><option>Blocked</option></select></label><button type="button" data-workspace-action="Step-up approval requested for ${person}">Verify &amp; save</button><button type="button" data-workspace-action="${person} deactivation requested">De-Activate</button><button class="danger-action" type="button" data-workspace-action="Delete ${person} confirmation opened">Delete</button></div>`;
  const staffActivation = () => Object.entries(PEOPLE).map(([level, people]) => `<details class="settings-disclosure"><summary><span><b>${level}</b>${guide(`Enter an employee name, activate the account, then assign task, salary and device access.`)}</span><label onclick="event.stopPropagation()"><input type="checkbox"> Activate</label></summary><div class="activation-add"><input aria-label="Employee name for ${level}" placeholder="Enter employee first and last name"><button type="button" data-workspace-action="${level} employee activation prepared">Activate employee</button></div>${people.map(([name, position]) => `<article class="employee-detail"><header><div><strong>${name}</strong><small>${position} · RBAC</small></div><span class="status-chip">Active</span></header>${staffControls(name)}</article>`).join("")}</details>`).join("");
  const accessRecord = ([name, position], includeDelete = true) => `<details class="access-record"><summary><span><strong>${name}</strong><small>${position}</small></span><span>Task</span><span>Permission</span><span>Authorize Access</span><span>Device</span><b>›</b></summary><div class="access-controls">${["Devices", "MFA", "Activated Staff", "Password Authorization", "Promotion enable"].map((item, i) => `<label><span>${item}${guide(i === 0 ? "Choose approved devices." : i === 1 ? "Require multi-factor authentication." : i === 2 ? "Activate or suspend this employee." : i === 3 ? "Reauthenticate before saving access." : "Enable a promotion request for owner approval.")}</span><input type="checkbox" ${i === 1 ? "checked" : ""}></label>`).join("")}<button type="button" data-workspace-action="Password reset email prepared for ${name}">Password Reset · Send Email</button>${includeDelete ? `<button class="danger-action" type="button" data-workspace-action="Delete ${name} confirmation opened">Delete</button>` : ""}</div></details>`;
  const employeeDirectory = () => Object.entries(PEOPLE).map(([level, people]) => `<details class="settings-disclosure"><summary><span><b>${level}</b>${guide("Open this group to view employee names, positions and RBAC assignments.")}</span><b>›</b></summary><div class="employee-list">${people.map(([name, position]) => `<article><span><strong>${name}</strong><small>${position}</small></span><button type="button" data-workspace-action="${name} RBAC opened">RBAC</button></article>`).join("")}</div></details>`).join("");
  const workspaceDirectory = () => Object.entries(PEOPLE).filter(([level]) => level !== "Upper Management").map(([level, people]) => `<details class="settings-disclosure"><summary><span><b>${level}</b>${guide("Open the group, choose an employee, and assign workspace and task access.")}</span><b>›</b></summary>${people.map(([name, position]) => `<details class="employee-workspace"><summary><span><strong>${name}</strong><small>${position} · RBAC</small></span><b>›</b></summary><div class="task-access"><label>Task<input placeholder="Assigned task" aria-label="Task assigned to ${name}"></label><label>Assigned<select aria-label="Assignment status for ${name}"><option>Assigned</option><option>Pending</option></select></label><label>Permission<select aria-label="Workspace permission for ${name}"><option>View</option><option>Edit</option><option>Manage</option></select></label><label>Device<select aria-label="Device for ${name}"><option>Approved device</option><option>Managed device</option></select></label><button type="button" data-workspace-action="Workspace access saved for ${name}">Assign access</button></div></details>`).join("")}</details>`).join("");
  const accessSummary = () => `<section class="access-summary" aria-label="User access summary"><article><strong>25</strong><span>Total users</span></article><article><strong>1</strong><span>Director</span></article><article><strong>2</strong><span>Managers</span></article><article><strong>2</strong><span>Supervisors</span></article><article><strong>20</strong><span>Staff</span></article><article><strong>2</strong><span>Teams</span></article></section>`;
  const userAccessRows = () => USERS.map(user => `<article class="user-access-row" data-user-row data-search="${user.name.toLowerCase()} ${user.id.toLowerCase()} ${user.role.toLowerCase()} ${user.team.toLowerCase()}" data-role="${user.role}" data-team="${user.team}"><div class="user-access-identity"><span class="user-avatar" aria-hidden="true">${user.name.split(" ").map(part => part[0]).slice(0, 2).join("")}</span><span><strong>${user.name}</strong><small>${user.id} · ${user.status}</small></span></div><span><b>${user.role}</b><small>Role</small></span><span><b>${user.team}</b><small>Team</small></span><span><b>${user.mfa ? "Required" : "Enrollment due"}</b><small>MFA</small></span><span><b>${user.device}</b><small>Device policy</small></span><div class="user-row-actions"><button type="button" data-workspace-action="Access review opened for ${user.name}">Review access</button><button type="button" data-workspace-action="${user.name} deactivation requested">Deactivate</button><button class="danger-action" type="button" data-workspace-action="Delete ${user.name} confirmation opened">Delete</button></div></article>`).join("");
  const userAccessPage = () => `<h3>User Access</h3><p>Manage exactly 25 tenant users across two teams. Roles provide job responsibility; permissions and approvals determine what each person can actually do.</p>${accessSummary()}<div class="access-purpose"><strong>Purpose</strong><span>Invite, assign, review, suspend or remove access without sharing an owner password. Privileged changes request fresh step-up authentication and backend approval.</span></div><div class="user-access-tools"><label><span>Find a user</span><input id="userAccessSearch" type="search" placeholder="Search name, ID, role or team"></label><label><span>Role</span><select id="userAccessRole"><option value="">All roles</option><option>Director</option><option>Manager</option><option>Supervisor</option><option>Staff</option></select></label><label><span>Team</span><select id="userAccessTeam"><option value="">Both teams</option>${TEAMS.map(team => `<option>${team}</option>`).join("")}</select></label><button type="button" data-workspace-action="User invitation prepared">Invite user</button></div><div class="team-summary">${TEAMS.map((team, index) => `<article><span class="team-number">0${index + 1}</span><span><strong>${team}</strong><small>${USERS.filter(user => user.team === team).length} assigned users</small></span><button type="button" data-workspace-action="${team} workspace opened">Open team</button></article>`).join("")}</div><div class="user-access-list" id="userAccessList">${userAccessRows()}</div><p class="user-access-empty" id="userAccessEmpty" hidden>No users match these filters.</p><div class="prototype-lock">Firebase target: Authentication identifies each person; Firestore stores tenant membership, teams and granular permissions; trusted server functions apply privileged changes, revoke sessions, notify affected users and write audit events.</div>`;
  const settingsPane = key => {
    if (key === "user-access") return userAccessPage();
    if (key === "my-profile") return `<h3>My Profile</h3><p>Manage the signed-in account owner’s identity, password, delegated access and trusted devices.</p><div class="setting-option-list">${compactOption("Name and Last Name", "Enter your legal or preferred first and last name, then save the profile.", "Edit")}${compactOption("Password", "Choose a new password of at least 16 characters with uppercase, lowercase, number and symbol.", "Change")}${compactOption("Access Delegated", "Review who can act on assigned work and remove access that is no longer required.", "Review")}${compactOption("MFA", "Enroll or verify a second authentication method for account protection.", "Configure")}${compactOption("Devices", "Review signed-in devices and revoke unfamiliar or inactive sessions.", "Review")}</div>`;
    if (key === "business-profile") return `<h3>Business Profile</h3><p>Maintain business identity, security and staff activation from one page.</p><form class="workspace-form"><div class="field-grid">${["Business Name", "Tax ID", "Address", "Phone"].map(field => `<label>${field}<input type="text" aria-label="${field}" placeholder="${field}">${guide(`Enter the current ${field.toLowerCase()} and save the business profile.`)}</label>`).join("")}</div><div class="form-actions"><button class="workspace-button" type="reset">Cancel</button><button class="workspace-button primary-action" type="submit">Save Business Profile</button></div></form><div class="setting-option-list security-shortcuts">${compactOption("Password", "Change the business-owner password using recent authentication.", "Change")}${compactOption("MFA", "Require MFA for the owner and privileged accounts.", "Configure")}${compactOption("Devices", "Review and revoke devices with business access.", "Review")}</div><section class="settings-subsection"><h4>Activate Staff</h4>${guide("Choose an employee level, enter the employee name, activate access and define task, salary and device permissions.")}${staffActivation()}</section>`;
    if (key === "access-security") return `<h3>Create Access and Security</h3><p>Configuration &amp; Authorized Access List</p>${guide("Open an employee record, select task and permission scope, authorize the device, then reauthenticate before saving.")}<div class="access-list">${Object.values(PEOPLE).flat().map(accessRecord).join("")}</div>`;
    if (key === "delegated-staff") return `<h3>Delegated Task · Staff Setting</h3><p>Employee List by employee name, last name and RBAC.</p>${guide("Open a management level, then choose an employee to review the assigned RBAC.")}${employeeDirectory()}`;
    if (key === "delegated-workspace") return `<h3>Delegated Task · Workspace</h3><p>Create a workspace and assign access to workspaces and tasks.</p><div class="workspace-create"><input aria-label="New workspace name" placeholder="Workspace name"><button type="button" data-workspace-action="Workspace creation prepared">Create Workspace</button></div>${guide("Create the workspace first, then open an employee and assign task, permission and device access.")}${workspaceDirectory()}`;
    if (key === "activity") return `<h3>Activity</h3><p>Recent login activity for employees with assigned access.</p>${guide("Review timestamps and open any unexpected login for investigation.")}<div class="activity-list">${Object.values(PEOPLE).flat().map(([name], i) => `<article><strong>${name}</strong><span>login: 09-03-${String(8 + i).padStart(2, "0")}-${String(12 + i * 3).padStart(2, "0")}</span><button type="button" data-workspace-action="${name} login activity opened">Review</button></article>`).join("")}</div>`;
    return `<h3>Payroll</h3><p>Authorize payroll work only for employees with an approved business need.</p>${guide("Open an employee, verify the task, permission, device and MFA, then complete fresh step-up authentication before approval.")}<div class="access-list">${Object.values(PEOPLE).flat().map(person => accessRecord(person, false)).join("")}</div>`;
  };
  ...
  const settingsMarkup = () => {
    ...
    return `<div class="settings-accordion" aria-label="Settings sections">${expandableSection("my-profile", "My Profile", "Personal identity, password, delegation, MFA and devices.", profile, true)}${expandableSection("business-profile", "Business Profile", "Business identity, contact information and protected confirmation.", business)}${expandableSection("add-employee", "Add New Employee +", "Employee information, reporting line, team and salary.", employee)}${expandableSection("app-access-rbac", "App Access & RBAC", "All screen privileges, working hours and role-based access.", access)}${expandableSection("contracts", "Contract & Invitation", "E-signature, delivery and acceptance confirmation.", contract)}${expandableSection("owner-approval", "Owner Approval", "Password, MFA, device and OAuth reauthentication evidence.", approval)}${expandableSection("password-reset", "Employee Password Reset", "Search an employee and send a secure reset email.", reset)}</div>`;
  };
  ...
  const render = (screen, requestedTab) => {
    ...
    if (screen === "settings") {
      ...
      const search = $("userAccessSearch"), role = $("userAccessRole"), team = $("userAccessTeam"), empty = $("userAccessEmpty");
      const filterUsers = () => {
        if (!search || !role || !team) return;
        const query = search.value.trim().toLowerCase();
        let visible = 0;
        host.querySelectorAll("[data-user-row]").forEach(row => {
          const show = (!query || row.dataset.search.includes(query)) && (!role.value || row.dataset.role === role.value) && (!team.value || row.dataset.team === team.value);
          row.hidden = !show;
          if (show) visible += 1;
        });
        if (empty) empty.hidden = visible !== 0;
      };
      [search, role, team].filter(Boolean).forEach(control => control.addEventListener("input", filterUsers));
